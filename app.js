...
let introSeen = false;
try { introSeen = sessionStorage.getItem('pk-intro-seen-v6') === '1'; } catch (e) {}
function closeIntro() {
  if (!intro || intro.classList.contains('intro-leaving')) return;
  intro.classList.add('intro-leaving');
  try { sessionStorage.setItem('pk-intro-seen-v6', '1'); } catch (e) {}
  setTimeout(() => { intro.remove(); document.body.classList.remove('intro-active'); document.querySelector('.brand-lockup')?.classList.add('logo-reveal'); }, 650);
}
if (intro) {
  if (introSeen) { intro.remove(); requestAnimationFrame(() => document.querySelector('.brand-lockup')?.classList.add('logo-reveal')); }
  else {
...
const baseRows = [
  ['trojan-wpc',6,'Trojan WPC','Multiwood','brand/trojan_wpc.png'],['thomson-multiwood',7,'Thomson Multiwood','Multiwood','brand/thomsom.png'],['eezeewood',8,'Eeezewood','Multiwood','brand/eezeewood.png'],['woodlane',9,'Woodlane','Multiwood','brand/wulane.png'],
  ['greenlam-laminates',10,'Greenlam Laminates','Laminates','brand/greenlam_laminates.png'],['merino-laminates',12,'Merino Laminates','Laminates','brand/merino_laminates.jpg'],['royale-touche',13,'Royale Touche','Laminates','brand/royale_touch.jpg'],['advance-laminates',14,'Advance Laminates','Laminates','brand/advance_laminate.png'],['sequence-surfaces',15,'Sequence Surfaces','Laminates','brand/sequence_surfaces.png'],
  ['mikasa-decowood',16,'Mikasa Decowood','Veneers','brand/decowood.png'],['century-veneers',17,'Century Veneers','Veneers','brand/century_veneer.jpeg'],
  ['hafele-hardware',18,'Häfele Hardware','Hardware','brand/hafele.webp'],['hettich-hardware',19,'Hettich Hardware','Hardware','brand/hettich.jpg'],['ebco-hardware',20,'Ebco Hardware','Hardware','brand/ebco.webp'],['sleek-hardware',21,'Sleek Hardware','Hardware','brand/sleek.jpg'],
  ['fevicol',22,'Fevicol','Adhesives','brand/fevicol.png'],['araldite',23,'Araldite','Adhesives','brand/araldite.png'],['flex-kwik',24,'Flex Kwik','Adhesives','brand/flexkwik.png'],
...
const removedProductIds = new Set(['hettich-hardware','sequence-surfaces','eezeewood','woodlane','araldite','flex-kwik','acrylic-01','acrylic-02','acrylic-03','acrylic-04','acrylic-05','acrylic-06']);
const baseProducts = baseRows.filter(r => !removedProductIds.has(r[0])).map(r => ({ id:r[0], legacyId:r[1], name:r[2], category:r[3], brand:r[2].split(' ')[0], image:ROOT + encodeURI(r[4]) }));
const mikasaBrand = { id:'brand-mikasa-plywood', name:'Mikasa Plywood', category:'Plywood', brand:'Mikasa', image:ROOT + encodeURI('brand/mikasa_ply.png'), isBrand:true, productCount:8 };
const plymarcBrand = { id:'brand-plymarc-plywood', name:'Plymarc Plywood', category:'Plywood', brand:'Plymarc', image:ROOT + encodeURI('brand/plymarc_ply.png'), isBrand:true, productCount:1 };
const trojanBrand = { id:'brand-trojan-plywood', name:'Trojan Plywood', category:'Plywood', brand:'Trojan', image:ROOT + encodeURI('brand/trojan.png'), isBrand:true, productCount:5 };
const plymarcTrustwudBrand = { id:'brand-plymarc-pvc-wpc', name:'Plymarc PVC & WPC', category:'Multiwood', brand:'Plymarc Trustwud', collectionKey:'Plymarc PVC & WPC', image:ROOT + encodeURI('brand/plymarc_wpc.png'), isBrand:true, productCount:3 };
const century08Products = window.CENTURY_CATALOGUE?.century08 || [];
const century10Products = window.CENTURY_CATALOGUE?.century10 || [];
const centuryBrand = { id:'brand-century-laminates', name:'Century Laminates', category:'Laminates', brand:'Century', collectionKey:'Century Laminates', image:ROOT + encodeURI('brand/century_laminates.jpg'), isBrand:true, productCount:century08Products.length + century10Products.length };
...
const hettichBrand = { id:'brand-hettich-hardware', name:'Hettich Hardware', category:'Hardware', brand:'Hettich', image:ROOT + encodeURI('brand/hettich.jpg'), isBrand:true, productCount:hettichProducts.length };
...
const brandCollections = { Hettich:hettichProducts, Mikasa:mikasaProducts, Plymarc:plymarcProducts, Trojan:trojanProducts, 'Plymarc PVC & WPC':plymarcTrustwudProducts, 'Century Laminates':centuryRangeCards, 'Century 1.0 mm':century10Products, 'Century 0.8 mm':century08Products };
const brandLabels = { Hettich:'Hettich Hardware', Mikasa:'Mikasa Plywood', Plymarc:'Plymarc Plywood', Trojan:'Trojan Plywood', 'Plymarc PVC & WPC':'Plymarc PVC & WPC', 'Century Laminates':'Century Laminates', 'Century 1.0 mm':'Century Laminates - 1.0 mm', 'Century 0.8 mm':'Century StarLine - 0.8 mm' };
...
const products = [...hettichProducts, ...displayFirstProducts, ...mikasaProducts, ...plymarcProducts, ...trojanProducts, ...plymarcTrustwudProducts, ...century10Products, ...century08Products, ...remainingBaseProducts];
...
const categories = ['All','Hettich','Hardware','Adhesives','Mikasa','Plymarc','Trojan','Plymarc PVC & WPC','Century Laminates','Laminates',...new Set(baseProducts.map(p => p.category).filter(c => !['Hardware','Adhesives','Laminates'].includes(c)))];
...
function renderProducts() {
  ...
  if(brandView){const collection=brandCollections[activeCategory];resultsBar.querySelector('strong').textContent=brandLabels[activeCategory]||activeCategory;resultsBar.querySelector('span').textContent=activeCategory==='Hettich'?`${collection.length} individual Hettich items extracted`:activeCategory==='Century Laminates'?'Choose 1.0 mm or 0.8 mm':`${collection.length} ${collection.length===1?'product':'official products'}`;}
  grid.innerHTML = shown.map(p => {
    if(p.isBrand){const countLabel=p.brand==='Hettich'?`${p.productCount} individual items extracted`:`${p.productCount} ${p.productCount===1?'product':'products'} extracted`;return `<article class="product-card brand-card"><div class="product-image"><span>${escapeHtml(p.category)}</span><img loading="lazy" src="${escapeHtml(p.image)}" alt="${escapeHtml(p.name)}"></div><div class="product-info"><small>${escapeHtml(countLabel)}</small><h3>${escapeHtml(p.name)}</h3><button class="add-button" data-brand="${escapeHtml(p.collectionKey||p.brand)}">View products →</button></div></article>`;}
...
document.querySelector('.whatsapp-button').addEventListener('click',()=>{
  ...
  window.open(`https://wa.me/917510210235?text=${encodeURIComponent(message)}`,'_blank','noopener');
});
document.addEventListener('keydown',e=>{if(e.key==='Escape'){if(document.body.classList.contains('spec-open'))closeSpec();else openCart(false);}});
